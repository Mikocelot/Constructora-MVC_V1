...
$encargado=isset($_POST["idencargado"])? limpiarCadena($_POST["idencargado"]):"";
...
switch ($_GET["op"]) {
	 case 'guardaryeditar':
			if (empty($idobra))
			{
				$rspta=$obra->insertar($nombreobra,$direccion,$nombre_due,$fechadeinicio,$fechadetermino,$costofinal,$encargado);
				echo $rspta ? "Obra Registrada" : "Obra no se pudo registrar";
			}
			else {
				$rspta=$obra->editar($idobra,$nombreobra,$direccion,$nombre_due,$fechadeinicio,$fechadetermino,$costofinal,$encargado);
 		...
 		$data= Array();
			while ($reg=$rspta->fetch_object()) {
				$data[] = array(
					"0"=>$reg->nombreobra,
	 				"1"=>$reg->direccion,
	 				"2"=>$reg->nombre." $reg->apellidoP $reg->apellidoM",
	 				"3"=>$reg->nombre_dueno,
	 				"4"=>$reg->fechadeinicio,
	 				"5"=>$reg->fechadetermino,
	 				"6"=>$reg->costofinal,
	 				"7"=>'<button class="btn btn-primary btn-sm" data-toggle="modal" data-target="#modal-nueva-obra" onclick="mostrar('.$reg->idobra.')"> <i class="material-icons">edit</i></button>'.
 					' <button class="btn btn-danger btn-sm" onclick="eliminar('.$reg->idobra.')"> <i class="material-icons">delete</i></button>'
				);
			}
			$results = array(
